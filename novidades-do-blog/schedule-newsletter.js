 ...
 *
 * Agenda um post do Ghost como newsletter "Novidades do Blog".
 *
...
function getScheduledDate() {
	...
	let scheduledISO = `${year}-${month}-${day}T${pad(SCHEDULE_HOUR)}:${pad(
		SCHEDULE_MINUTE
	)}:00${offset}`
...
async function main() {
	...
	console.log(`\n=== Agendando Newsletter Novidades do Blog ===\n`)
	...
	const tags = hasTag ? existingTags : [...existingTags, { name: TAG_NAME }]

	const editResult = await ghostPut(
		`posts/${postId}/`,
		{
			posts: [
				{
